...
export class UserCommand extends RTByteCommand {
	public async run(message: Message, args: RTByteCommand.Args) {
		...
		if (!input.success) return message.reply(args.t(LanguageKeys.Commands.User.EnlargeNoneSpecified))
		...
		if (!emoji && !customEmoji) return message.reply(args.t(LanguageKeys.Commands.User.EnlargeNoneSpecified))

		const emojiAttachment = new MessageAttachment(emoji
			? emoji.url
			: `https://cdn.discordapp.com/emojis/${customEmoji}.${animated ? 'gif' : 'png'}`);
		return message.reply({files: [emojiAttachment] });
	}
}
