...
module.exports = class extends Command {
	...
	async run(msg, [username, when = null, ...reason]) {
		...
		if (!msg.guild.settings.roles.muted || !msg.guild.roles.cache.has(msg.guild.settings.roles.muted)) await this.createRole(msg.guild);
		...
		if (member.roles.cache.has(msg.guild.settings.roles.muted)) return msg.affirm();
		const mutedRole = await msg.guild.roles.cache.get(msg.guild.settings.roles.muted);
