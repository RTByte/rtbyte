...
module.exports = class extends Command {
	...
	async run(msg, [username, when = null, ...reason]) {
		...
		if (!msg.guild.settings.get('roles.voiceBanned') || !msg.guild.roles.cache.has(msg.guild.settings.get('roles.voiceBanned'))) await this.createRole(msg.guild);
		...
		if (member.roles.cache.has(msg.guild.settings.get('roles.voiceBanned'))) return msg.affirm();
		const voiceBannedRole = await msg.guild.roles.cache.get(msg.guild.settings.get('roles.voiceBanned'));
