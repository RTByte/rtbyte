...
module.exports = class extends Command {
	...
	async run(msg, [membername = msg.member]) {
		...
		const embed = new MessageEmbed()
			...
			.addField(msg.guild.language.get('NAME'), membername.user.tag, true)
			.addField(msg.guild.language.get('ID'), membername.id, true)
			.addField(msg.language.get('JOIN_POS'), position)
			.addField(msg.guild.language.get('JOINED'), `${this.timestamp.displayUTC(membername.joinedTimestamp)} (UTC)`, true)
			.addField(msg.guild.language.get('REGISTERED'), `${this.timestamp.displayUTC(membername.user.createdAt)} (UTC)`, true)
