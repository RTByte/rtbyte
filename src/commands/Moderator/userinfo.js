...
module.exports = class extends Command {
	...
	async run(msg, [membername = msg.member]) {
		...
		if (membername.user.presence.activity) {
			...
			await embed.addField(!membername.user.presence.activity.type ? membername.user.presence.activity.name : msg.guild.language.get('COMMAND_USERINFO_ACTIVITY', membername), !membername.user.presence.activity.type ? membername.user.presence.activity.state || 'N/A' : membername.user.presence.activity ? !membername.user.presence.activity.details ? membername.user.presence.activity.name : membername.user.presence.activity.type === 'LISTENING' ? `${membername.user.presence.activity.name}\n\`${membername.user.presence.activity.details} by ${membername.user.presence.activity.state}\`` : `${membername.user.presence.activity.name}\n\`${membername.user.presence.activity.details}\`` : 'N/A', true);
		}
