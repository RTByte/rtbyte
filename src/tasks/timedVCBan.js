...
module.exports = class extends Task {
	...
	async run({ guildID, userID, modID }) {
		...
		if (!mod) return;
		const voiceBannedRole = await guild.roles.cache.get(guild.settings.get('roles.voiceBanned'));
		if (!member.roles.cache.has(guild.settings.get('roles.voiceBanned'))) return;
