...
export class UserEvent extends Listener {
	public async run(thread: ThreadChannel) {
		...
		const executor = await getAuditLogExecutor(AuditLogEvent.ThreadCreate, thread.guild);
	...
	private generateGuildLog(thread: ThreadChannel, executor: User | null | undefined, isForumThread: boolean) {
		...
		const embed = new GuildLogEmbed()
			...
			.setFooter({ text: `${postOrThread} created ${isNullish(executor) ? '' : `by ${executor.username}`}`, iconURL: isNullish(executor) ? undefined : executor?.displayAvatarURL() })
